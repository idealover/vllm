        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for role, message in self.messages:
                if message:
                    ...
                    ret += role + ":"
            return ret
        elif self.sep_style == SeparatorStyle.CHAT_STYLE:
            ret = system_prompt + self.sep
            for role, message in self.messages:
                if message: 
                    ret += role + ": " + message + self.sep
            ...
            #Add functions as well 
            if self.functions != ():
                ret = system_prompt + " Here are the available functions: \n\n"
...
register_conv_template(
    Conversation(
        name = "chat_template",
        system_message = "Reply like the user",
        roles = ("User","Others"),
        messages = (),
        offset = 0,
        sep_style = SeparatorStyle.CHAT_STYLE,
        sep = "</s>",
        sep2 = "</s>",
    )
)
